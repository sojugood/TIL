...
def create_user(n):
    ...
    count_list = []
    for i in n:
        if is_validation(i) == True:
            user_list.append(i)
        elif is_validation(i) == False:
            user_list.append(i)
            count_list.append('F')
        else:
            count_list.append('B')
    print(f"\'잘못된 데이터로 구성된 유저의 수는 {len(count_list)} 입니다.\'")
    return user_list
...
def is_validation(i):
    ...
    if i['company'] in black_list:
        ...
        if i['blood_group'] in blood_types:
            pass
        else:
            data_list.append('blood_group')
            i['blood_group'] = None
        if '@' in i['mail']:
            pass
        else:
            data_list.append('mail')
            i['mail'] = None
...
pprint.pprint(create_user(user_data))